...
2. **`/api/*` routing** — serialize the request, hand it to the dedicated Worker
   via a `MessageChannel` port (received from the main thread at boot), wait for
   the matching response, return it.
...
The plugin serves `sw.js` from the package source in `vite dev` and emits it at
the build root for `vite build`. Consumers must register it from the main thread
