...
// IMPORTANT: drizzle-orm has TWO libsql entry points — `drizzle-orm/libsql`
// imports from `@libsql/client` (Node), while `drizzle-orm/libsql/wasm`
// imports from `@libsql/client-wasm` (browser). Pick the wasm one.
...
// `file:` URL maps to OPFS in @libsql/client-wasm (per libsql-wasm-experimental
// docs — leading slash is optional). Persistence across reloads is the key
// test: if data survives, OPFS is being used; if not, it's in-memory.
const DB_URL = "file:spike-01b.db";
...
async function main() {
  await reportEnv();
  try {
    log(`opening libsql client at ${DB_URL}`);
