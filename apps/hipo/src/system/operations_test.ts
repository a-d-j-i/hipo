...
Deno.test(
  "system.status: schema version + framework version present",
  async () => {
...
Deno.test(
  "system.status: Deno runtime reports shape=server + libsql-local",
  async () => {
    const ctx = await freshCtx();
    const s = await do_getSystemStatus(ctx);
...
Deno.test(
  "system.status: server shape clears cleared_by_browser_data_clear",
  async () => {
    const ctx = await freshCtx();
    const s = await do_getSystemStatus(ctx);
...
Deno.test(
  "system.status: empty target table flags no_backup_configured",
  async () => {
    const ctx = await freshCtx();
    const s = await do_getSystemStatus(ctx);
...
Deno.test(
  "system.status: recent verified backup → survives_device_loss_via_backup=true",
  async () => {
    const ctx = await freshCtx();
...
Deno.test(
  "system.status: stale backup → no_recent_backup=true, survives=false",
  async () => {
    const ctx = await freshCtx();
    const now = Math.floor(Date.now() / 1000);
    await ctx.db.insert(backupTargetState).values({
...
Deno.test(
  "system.status: failed last verify → last_verify_failed=true",
  async () => {
    const ctx = await freshCtx();
    const now = Math.floor(Date.now() / 1000);
    await ctx.db.insert(backupTargetState).values({
...
Deno.test(
  "system.status: write-only target (no verify) still counts as recent",
  async () => {
    const ctx = await freshCtx();
    const now = Math.floor(Date.now() / 1000);
